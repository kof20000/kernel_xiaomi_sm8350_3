...
#include <oplus/qcom_platform_lahaina.h>
...
static int get_platform(void)
{
	...
	if (strstr(comp_str, PLATFORM_LAHAINA)) {
		platform_id = LAHAINA;
...
static int ws_fb_notify_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	if (evdata && evdata->data) {
		...
		if (*blank == MI_DISP_DPMS_POWERDOWN) { /*suspend*/
			if (event == MI_DISP_DPMS_EARLY_EVENT) { /*early event*/
				pr_info("[%s], POWERDOWN.\n", __func__);
			}
		} else if (*blank == MI_DISP_DPMS_ON) { /*resume*/
			if (event == MI_DISP_DPMS_EARLY_EVENT) { /*event*/
				pr_info("[%s], UNBLANK\n", __func__);
			}
		}
...
static int __init wakelock_statistics_function_init(void)
{
	...
	retval = mi_disp_register_client(&ws_fb_notify_block);
	if (retval) {
